---
title: Interest signal collection — 19:00 social/browser run (2026-07-14)
created: 2026-07-14
type: raw-transcript
captured_at: 2026-07-14T19:05:00+09:00
run_tag: interest-signal-collection-19-00
track: social
tags: [raw-notes, threads, threads-api, social, cdp-unavailable, reinforcement]
route_hint: raw_only
route_confidence: high
route_reason: >
  Afternoon social run. Baseline = prior social run (2026-07-13 08:00, both-static
  no-new-signal). Live Chrome CDP on 127.0.0.1:9222 again unavailable
  ...
  authenticated source. Against the 07-13 08:00 baseline, own /threads is UNCHANGED
  (newest still DapQbi9k_Vk, 07-11 07:43 UTC) but /replies gained ONE new dated row —
  Dato11Qk5Z4 (2026-07-13 00:33 UTC), a soccer opinion reply ("축구가 무슨 자아니 에고니
  따지면서하냐"), off-axis noise. This is the /replies freshness asymmetry (CK replies
  more than he posts). mentions empty. keyword_search (agent / Claude Code / Codex)
  again returned only CK's own April–May managed-agents posts + the 07-10 "codex는
  없나요?" reply (reinforcement of an already-durable axis). Net = reinforcement-only;
  the one new /replies row is single-surface off-axis noise → raw_only. No durable
  page change.
routing:
  - {surface: threads-api-own-threads, route: raw_only, note: "UNCHANGED since 07-13 08:00 baseline — newest still DapQbi9k_Vk (07-11 07:43 UTC, empty REPOST_FACADE). No new own posts/reposts."}
  - {surface: threads-api-replies, route: raw_only, note: "ONE new dated row since baseline — Dato11Qk5Z4 (2026-07-13 00:33 UTC) soccer opinion reply, off-axis noise. All other rows repeat prior baselines."}
...
# Raw — 19:00 Social/Browser Collection (2026-07-14)

Source: Threads API (official graph.threads.net); live Chrome CDP browser surfaces
Date: 2026-07-14 (captured 19:05 KST)
...
- No same-day 19:00 raw transcript existed before this run → proceeded with collection. (No 07-14 08:00 raw exists either; the last completed social run was 07-13 08:00, used as baseline.)
- Threads long-lived token refreshed OK (`/refresh_access_token` returned a fresh token, expires_in ~5.18M s); `/v1.0/me` confirmed identity (`ethan13917`, `씨케이`). Raw JSON cached at `~/wiki/.cache/threads-api-collection-19-00-2026-07-14.json` (disposable).
- Cron-mode execution: standalone `/tmp/th_collect_1900_0714.py` run via `python3 <path>` using stdlib `urllib` (execute_code / heredoc / `-c` / pipe-to-interpreter blocked in cron).

## 1) Threads API — own `/threads` (copied) — NO NEW ROWS since 07-13 08:00
Source URL: https://www.threads.com/@ethan13917
Newest row is identical to the 07-13 08:00 baseline; no own posts/reposts since then.
...
> (older 07-09 → 06-29 repost + soccer/bitcoin/civic rows all unchanged from baselines: DalhLj5E7Xd, DakUpdgE2cy, DaO-1ShE7lv 비트코인, DaLICr2k-hU 태극기, ...)

## 2) Threads API — own `/replies` (copied) — ONE new dated row since 07-13 08:00
Source URL: https://www.threads.com/@ethan13917/replies
Newest reply is NEW vs the 07-13 08:00 baseline (which topped at DapS3RAE4dA, 07-11 08:04).
> `[2026-07-13T00:33:38Z]` https://www.threads.com/@ethan13917/post/Dato11Qk5Z4 "축구가 무슨 자아니 에고니 따지면서하냐? 축구는 그냥하는거야. 축구의신도 축구는 그냥 하는거야. 대회라면 결의를 다지는 정도겠지" (**NEW** — soccer opinion; off-axis noise)
...
> (all other 07-10/07-07/07-06/07-05 replies — military(7사단 GOP)/soccer(홍명보,이강인,김민재,백승호)/personal/entertainment — unchanged from baselines; noise)
...
## Filtering notes
- Excluded from the report as noise: all soccer/홍명보/이강인/김민재/백승호 posts and replies, military(7사단 GOP), bitcoin/태극기/civic rows, entertainment/personal replies.
- The one genuinely new row (07-13 00:33 soccer reply Dato11Qk5Z4) is off-axis noise → `raw_only`, kept as chronology evidence only.
- keyword-search rows are CK's own old posts, not fresh discovery — reinforcement.

## Why this raw exists
Evidence store for the 2026-07-14 19:00 social pass. Supports a `raw_only` /
reinforcement-only outcome: CDP-authenticated surfaces were unavailable, and the only
grounded source (Threads API) showed own `/threads` unchanged since the 07-13 08:00
baseline, with `/replies` gaining one new off-axis soccer reply and keyword-search
returning only CK's own durable managed-agents posts. Durable action = raw transcript +
Honcho audit + log entry; `index.md` and non-Honcho concept pages left unchanged.
