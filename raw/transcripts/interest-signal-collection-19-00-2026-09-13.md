---
title: Interest signal collection — 19:00 social run (2026-09-13)
type: raw
created: 2026-09-13
captured_at: 2026-09-13T19:10 KST
...
route_reason: >
  ...
  interest-signal-collection-19-00-2026-09-13 raw existed before this run (same-day 08:00 social +
  11:00 new-target present). Baselines = same-day 08:00 social (both-static no-new-signal) + 09-12 19:00
  social. Live Chrome CDP UP (Chrome/147.0.7727.55). Threads via official Graph API (configured path);
  token refresh OK (expires_in 5171391), /v1.0/me OK (ethan13917, id 27173231028946699). GitHub stars +
  ...
  (1) Threads: own /threads top row UNCHANGED vs the 08:00 baseline (still `18084391358315488`
  ...
  reinforcement. (2) GitHub stars 94, UNCHANGED (cdp body len 2807 byte-identical to 08:00, top simonw/llm)
...
# Raw Transcript — 2026-09-13 19:00 social/browser interest-signal run
...
## Live session notes
- **Live Chrome CDP `127.0.0.1:9222` UP (Chrome/147.0.7727.55).** Read hydrated `type==page` tabs via stdlib raw-socket WS CDP client (`/tmp/cdp_read_1900_0913.py`). Raw JSON cached `~/wiki/.cache/cdp_0913_1900.json` (+ YouTube longer tab `cdp_0913_1900_yt.json`). Full-body lens byte-identical to the 08:00 baseline: GitHub 2807, YouTube 9487, Google 5822. (First YouTube target `AD349…` was a blank shell len 17; fell back to the hydrated `2D6AE…` subs tab len 9487.)
- **Threads collected via official Graph API** (configured path), NOT CDP. Token refresh OK (expires_in 5171391); `/v1.0/me` OK (`ethan13917`, id 27173231028946699). Collector `/tmp/th_collect_1900_0913.py` (stdlib urllib), raw JSON cached `~/wiki/.cache/threads_0913_1900.json`.
- **Baselines** = same-day 08:00 social (both-static no-new-signal) + 09-12 19:00 social. No `interest-signal-collection-19-00-2026-09-13` raw existed before this run → idempotency guard clear.

## Novelty summary
**All-static no-new-signal.** Own `/threads` top row AND `/replies` top row both byte-identical to the 08:00 baseline (CK made no posts between passes). GitHub stars 94 / Google My Activity / YouTube subs all byte-identical (len 2807 / 5822 / 9487). keyword_search = CK's own older posts (reinforcement). Durable action = raw transcript + Honcho audit + `log.md` entry only.
...
### /replies — top row UNCHANGED since 08:00 → 변화 없음
- Top still `17956597608249422` (2026-09-12T04:36:29 UTC / 13:36 KST, TEXT_POST): "Macos 밀어버리시고 설치하신건가요?" — personal/tech-support reply, already captured 08:00 & 09-12 19:00. No new dated reply since.
...
Page title `Your stars`, https://github.com/stars, **94 stars** (body len 2807, byte-identical to the 08:00 baseline). Top row still `simonw/llm`. Recent-starred list + language breakdown unchanged.

## 3) Google My Activity (CDP) — len 5822 IDENTICAL → 변화 없음
`Google - 내 활동`, https://myactivity.google.com/search-services/history/search, len 5822 (byte-identical to the 08:00 baseline). No new 09-13 afternoon row.

## 4) YouTube subscriptions (CDP) — len 9487 IDENTICAL → 변화 없음
Hydrated (`구독 - YouTube`, len 9487, byte-identical to the 08:00 baseline). No new AI/agent/infra title.
- Off-axis noise kept in raw, excluded from report: 헬스/soccer/politics/crypto rows.

## Filtering notes
- Threads `/threads` static vs 08:00; `/replies` static vs 08:00 (both tops identical) → genuine no-new-signal.
