---
title: Interest signal collection — 19:00 social/browser run (2026-07-12)
created: 2026-07-12
type: raw-transcript
captured_at: 2026-07-12T19:03:00+09:00
...
route_reason: >
  Afternoon social run after same-day 08:00 social (no-new-signal) and 11:00 new-target
  (reinforcement-only) baselines. Live Chrome CDP on 127.0.0.1:9222 again unavailable
  (/json/version and /json/list connection-refused) — CDP down all day — so all
  ...
  Against BOTH the same-day 08:00 baseline AND the prior social run (07-11 19:00), own /threads
  is UNCHANGED (newest still DapQbi9k_Vk, 07-11 07:43 UTC) and /replies is UNCHANGED (newest
  still DapS3RAE4dA, 07-11 08:04 UTC) — no new own posts or replies. mentions empty.
...
routing:
  - {surface: threads-api-own-threads, route: raw_only, note: "UNCHANGED since the 08:00 baseline and the 07-11 19:00 social baseline — newest still DapQbi9k_Vk (07-11 07:43 UTC, empty REPOST_FACADE reposting @_joojinwoo, civic/political) and Daonr3ekwwK (07-11 01:47 UTC, reposts @sooneun1777 personal). No new rows. All rows repeat prior baselines."}
  - {surface: threads-api-replies, route: raw_only, note: "UNCHANGED since the 08:00 baseline and the 07-11 19:00 baseline — newest still DapS3RAE4dA (07-11 08:04, civic/police-commentary noise). No new dated rows. The on-axis 'codex는 없나요?' (07-10) was already captured 07-10 19:00."}
...
# Raw — 19:00 Social/Browser Collection (2026-07-12)

Source: Threads API (official graph.threads.net); live Chrome CDP browser surfaces
Date: 2026-07-12 (captured 19:03 KST)
...
- Baselined against the same-day 08:00 social run and the prior social run (07-11 19:00).
- Threads long-lived token refreshed OK (`/refresh_access_token` returned a fresh token); `/v1.0/me` confirmed identity (`ethan13917`, `씨케이`). Raw JSON cached at `~/wiki/.cache/threads-api-collection-19-00-2026-07-12.json` (disposable).
- Cron-mode execution: standalone `/tmp/th_collect_1900_0712.py` run via `python3 <path>` (execute_code / heredoc / `-c` / pipe-to-interpreter blocked in cron).

## 1) Threads API — own `/threads` (copied) — NO NEW ROWS since the 08:00 baseline
Source URL: https://www.threads.com/@ethan13917
Newest row is identical to the 08:00 baseline (and the 07-11 19:00 social baseline); no own posts/reposts since then.
...
> `[2026-07-09 → 07-02 repost + soccer rows]` all unchanged from baselines (DalhLj5E7Xd, DakUpdgE2cy, Daj1jPakxXo, DaiHwK9E-oR, DaedgJDk8Kx, DaeAEVik9gO, DaaVBUmk8Px TEXT_POST 축구, DaXLBO-EzG-, DaUrPU_E3MD, DaT4B85kwSZ, DaT1CAIk5l8)

## 2) Threads API — own `/replies` (copied) — NO NEW ROWS since the 08:00 baseline
Source URL: https://www.threads.com/@ethan13917/replies
Newest reply is identical to the 08:00 baseline (and the 07-11 19:00 baseline); no new dated rows.
...
- No new own posts or replies appeared since the 08:00 baseline — both surfaces static (both-static → clean no-new-signal case).
- keyword-search rows are CK's own old posts, not fresh discovery — reinforcement.

## Why this raw exists
Evidence store for the 2026-07-12 19:00 social pass. Supports a `raw_only` /
...
the 08:00 baseline and the 07-11 19:00 social baseline, with keyword-search returning only
CK's own durable managed-agents posts. Durable action = raw transcript + Honcho audit + log
