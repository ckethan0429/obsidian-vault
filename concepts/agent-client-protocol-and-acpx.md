...
sources: [raw/articles/acp-and-acpx-2026-04-15.md, https://github.com/openclaw/acpx#readme, raw/transcripts/interest-signal-collection-08-00-2026-04-22.md, raw/transcripts/interest-signal-collection-08-00-2026-04-22-threads-liked-rerun.md, raw/transcripts/interest-signal-collection-19-00-2026-04-22.md]
---
...
### 4. Hermes/Claude Code/Codex와 IDE의 관계
CK와의 대화에서 정리된 실무 감각은 이렇다:
- 대부분의 IDE는 이미 터미널을 내장하고 있으므로, Claude Code나 Codex는 IDE 터미널에서 바로 실행해도 충분한 경우가 많다.
- ACP는 "터미널로도 되는 작업"을 대체하기보다, IDE가 에이전트를 **첫 번째 클래스 기능**처럼 다루게 해주는 통로에 가깝다.
- IDE가 ACP를 통해 얻는 이점은 현재 열린 파일, 선택 영역, 커서 위치, 진단 결과, diff 같은 **편집기 맥락**을 에이전트에게 자연스럽게 넘길 수 있다는 점이다.
- Hermes는 상위 오케스트레이터로서 Claude Code/Codex를 CLI, subprocess, PTY, delegate-task 형태로 지휘할 수 있지만, 그 자체가 곧바로 Claude Code/Codex를 ACP 타깃으로 붙인다는 뜻은 아니다.
- 따라서 실무에서는 **터미널 연동 = 가장 단순하고 흔한 경로**, **ACP 연동 = IDE 안에서 더 깊은 통합이 필요할 때 선택하는 경로**로 보는 게 맞다.

### 5. 이 구조의 대표 유즈케이스
- IDE 안에서 현재 파일 또는 선택 영역만 바로 수정 요청하기
- 커서 위치와 진단 에러를 바탕으로 리팩터링/버그 수정 시키기
- 에이전트가 만든 diff를 IDE에서 바로 검토하고 accept/reject 하기
- Hermes가 작업을 쪼개서 Claude Code에는 리팩터링, Codex에는 테스트 작성처럼 **벤더 혼합 오케스트레이션** 하기
- 장시간 작업을 CLI/PTY로 돌리되, 필요할 때만 IDE와 깊게 연결하기
...
## 2026-04-22 live runtime reinforcement
The 2026-04-22 raw runs sharpened the page in two ways.

### 1. ACP server mode is useful, but not a durable state layer by itself
The visible ChatGPT `ACP Editor Integration` discussion kept repeating the same boundary:
- Hermes can run as an **ACP server**
- the integration is **editor-native** and useful for in-workspace execution
- but ACP sessions stay **in-memory per running server**
- therefore long-running automation still needs a **separate durable state layer**

That makes ACP more valuable as a control-plane interface than as the memory system itself.

### 2. Shared memory wiring matters more than the individual components
The same day's `공유 메모리 에이전트` excerpts repeatedly emphasized that the hard part is not `mem0`, `Qdrant`, or `Ollama` in isolation. The hard part is:
- making multiple agents and sessions read/write the **same memory layer**
- using **Stop hook** or equivalent lifecycle hooks to broadcast work traces
- pinning providers explicitly
- keeping idempotency and local fallback behavior predictable

In practice, this means ACP/ACPX sits best as the **session and control interface**, while a separate shared-memory layer handles continuity across tools.

### 3. Why this matters for CK
For CK's stack, the durable split now looks clearer:
- **ACP / ACPX** = how clients and runtimes control agent sessions
- **shared memory wiring** = how multiple sessions keep continuity
- [[honcho]] = how captured evidence is triaged before promotion
- [[managed-agents-practical-summary]] = how restartability, isolation, and verification turn that into an operating workflow

So the most practical reading is not "ACP replaces everything," but "ACP is the structured control surface that becomes much more powerful once it is paired with durable memory routing and restartable runtime design."
